import { ILayout } from "../models/Building";

/**
 * Room 101 - Default Layout Template
 * Standard classroom layout used as default for new rooms
 * 
 * Assets:
 * - 2 Boards (top center)
 * - 2 Projectors (below boards)
 * - 4 ACs (corners)
 * - 4 Fans (corners)
 * - 20 Tables (4 rows x 5 columns)
 * - 20 Chairs (paired with tables)
 */
export const getDefaultLayout1 = (): ILayout => {
  return {
    layoutType: "layout1",
    gridRows: 14,
    gridCols: 22,
...
export const getEmptyLayout = (): ILayout => {
  return {
    layoutType: "custom",
    gridRows: 14,
    gridCols: 22,
